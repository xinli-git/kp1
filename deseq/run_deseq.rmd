...
load sample file
```{r}
samples <- read.table(file.path("~/projects/kp1/sample/", "kp1_sample_table.txt"), header=TRUE)
```

```{r}
library("tximport")
library("readr")
dir <- "~/projects/kp1/rsem/rsem_output/"
files <- file.path(dir, "rsem", samples$run, paste0(samples$run, ".Aligned.toTranscriptome.genes.results"))
